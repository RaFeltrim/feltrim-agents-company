...
class TechLeadPlan(BaseModel):
    files: List[FileTask] = Field(description="List of files to create")
...
class BaseAgent:
    ...
    def _load_persona(self, agent_file: str) -> str:
        """Loads the persona markdown from feltrim-agents-base."""
        if agent_file in self.persona_cache:
            return self.persona_cache[agent_file]
            
        # Path relative to crewai-service/agents.py -> ../../feltrim-agents-base/core/agents/
        base_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "feltrim-agents-base", "core", "agents"))
        file_path = os.path.join(base_path, agent_file)
        ...
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                self.persona_cache[agent_file] = content
                return content
        except Exception as e:
            self.log(f"⚠️ Warning: Could not load persona {agent_file}. Using generic fallback. Error: {e}")
            return f"You are an expert AI agent mimicking {agent_file}."
...
class ProductManagerAgent(BaseAgent):
    def analyze_requirements(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.log(f"Claudia PM: Analisando '{state.get('project_name')}'...")
        self._log_thought("Lendo a descricao do projeto e extraindo user stories/requisitos...")
        
        persona = self._load_persona("gem_claudia_pm.md")
...
class SoftwareArchitectAgent(BaseAgent):
    def design_architecture(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.log("Beatriz TL (Architect): Projetando arquitetura...")
        self._log_thought("Analisando requisitos para determinar tech stack ideal...")
        
        persona = self._load_persona("gem_beatriz_tl.md")
        system_prompt = f"""{persona}
...
class TechLeadAgent(BaseAgent):
    def breakdown_tasks(self, state: Dict[str, Any]) -> Dict[str, Any]:
        self.update("plan", "running", {})
        self.log("Beatriz TL: Planejando arquivos (Structured Output)...")
        self._log_thought("Decompondo arquitetura em arquivos usando Pydantic...")
        
        persona = self._load_persona("gem_beatriz_tl.md")
        parser = PydanticOutputParser(pydantic_object=TechLeadPlan)
        ...
        try:
            # Safely parse JSON via LangChain
            plan = parser.parse(response)
            files = [task.model_dump() for task in plan.files]
        except Exception as e:
            self.log(f"Beatriz TL Warning: JSON parse falhou. Fallback. Erro: {e}")
            files = [{"path": "README.md", "purpose": "Documentation"}]
            
        self.log(f"Beatriz TL: Planejou {len(files)} arquivos com precisao estruturada.")
        return {"tasks": files}
...
class DeveloperAgent(BaseAgent):
    def write_code(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.log("Joao Backend / Fabio Frontend: Iniciando codigo...")
        self._log_thought("Implementando arquivos planejados...")
        
        tasks = state.get("tasks", [])
        requirements = state.get("requirements", "")
        architecture = state.get("architecture", "")
        
        persona = self._load_persona("gem_joao_backend.md")
        ...
        for i, file_info in enumerate(tasks):
            ...
            self.log(f"Developer: Escrevendo {file_path}...")
            self._log_thought(f"Implementando {file_path} - {purpose}")
...
class QAEngineerAgent(BaseAgent):
    ...
    def _test_node_project(self, temp_dir: str) -> Dict[str, Any]:
        """Test a Node.js project inside a Docker container."""
        
        # Step 1: npm install
        self.log("📦 Rafael QA: Rodando npm install no Docker (node:18-alpine)...")
        self._log_terminal("$ docker run node:18-alpine npm install")
        
        # Convert Windows path to Docker format if necessary
        docker_vol = temp_dir.replace("\\", "/")
        
        try:
            install_result = subprocess.run(
                ["docker", "run", "--rm", "-v", f"{docker_vol}:/app", "-w", "/app", "node:18-alpine", "npm", "install"],
                capture_output=True,
                text=True,
                timeout=120,
                shell=False
        ...
        self.log("🚀 Rafael QA: Rodando npm start no Docker (teste rapido)...")
        self._log_terminal("$ docker run node:18-alpine npm start")
        
        try:
            # Use 'timeout' inside alpine to stop the server after a few seconds
            start_result = subprocess.run(
                ["docker", "run", "--rm", "-v", f"{docker_vol}:/app", "-w", "/app", "node:18-alpine", "sh", "-c", "timeout 15 npm start"],
                capture_output=True,
                text=True,
                timeout=25,
                shell=False
    ...
    def _test_python_project(self, temp_dir: str) -> Dict[str, Any]:
        ...
        if os.path.exists(req_file):
            self.log("📦 Rafael QA: Instalando dependencias Python no Docker...")
            self._log_terminal("$ docker run python:3.11-alpine pip install")
            try:
                subprocess.run(
                    ["docker", "run", "--rm", "-v", f"{docker_vol}:/app", "-w", "/app", "python:3.11-alpine", "pip", "install", "-r", "requirements.txt", "-q"],
                    capture_output=True,
                    timeout=60,
                    shell=False
                )
        ...
        self.log(f"🐍 Rafael QA: Executando {main_file} no Docker...")
        self._log_terminal(f"$ docker run python:3.11-alpine python {main_file}")
        
        try:
            result = subprocess.run(
                ["docker", "run", "--rm", "-v", f"{docker_vol}:/app", "-w", "/app", "python:3.11-alpine", "sh", "-c", f"timeout {self.EXECUTION_TIMEOUT} python {main_file}"],
                capture_output=True,
                text=True,
    ...
    def _fix_code(self, file_path: str, content: str, error: str, 
        ...
        persona = self._load_persona("gem_rafael_qa.md")
